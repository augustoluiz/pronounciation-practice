INSERT INTO UNIDADE (ID, DESCRICAO, NOME)
VALUES (1, 'Primeira Unidade', 'Unit I');

INSERT INTO AUDIO (ID, DESCRICAO, LINK, TITULO)
VALUES (1, 'VIDEO 1 DA UNIT 1', 'https://www.youtube.com/embed/12GiF4EWrcQ', 'The Batman');

INSERT INTO AUDIO (ID, DESCRICAO, LINK, TITULO)
VALUES (2, 'VIDEO 2 DA UNIT 1', 'https://www.youtube.com/embed/12GiF4EWrcQ', 'The Batman2');

INSERT INTO AUDIO (ID, DESCRICAO, LINK, TITULO)
VALUES (3, 'VIDEO 3 DA UNIT 1', 'https://www.youtube.com/embed/12GiF4EWrcQ', 'The Batman3');

INSERT INTO AUDIO (ID, DESCRICAO, LINK, TITULO)
VALUES (4, 'VIDEO 4 DA UNIT 1', 'https://www.youtube.com/embed/12GiF4EWrcQ', 'The Batman4');

INSERT INTO AUDIO (ID, DESCRICAO, LINK, TITULO)
VALUES (5, 'VIDEO 5 DA UNIT 1', 'https://www.youtube.com/embed/12GiF4EWrcQ', 'The Batman5');

INSERT INTO EXERCICIO (ID, ORDEM_APRESENTACAO, TITULO, UNIDADE_ID)
VALUES (1, 1, 'EXERCICIO 1 - UNIT 1', 1);

INSERT INTO QUESTAO (ID, ORDEM_APRESENTACAO, TEXTO, EXERCICIO_ID)
VALUES (1, 3, 'EXERCICIO 1 - QUESTAO 3', 1);

INSERT INTO QUESTAO (ID, ORDEM_APRESENTACAO, TEXTO, EXERCICIO_ID)
VALUES (2, 2, 'EXERCICIO 1 - QUESTAO 2', 1);

INSERT INTO QUESTAO (ID, ORDEM_APRESENTACAO, TEXTO, EXERCICIO_ID)
VALUES (3, 1, 'EXERCICIO 1 - QUESTAO 1', 1);

INSERT INTO QUESTAO (ID, ORDEM_APRESENTACAO, TEXTO, EXERCICIO_ID)
VALUES (4, 4, 'EXERCICIO 1 - QUESTAO 4', 1);

INSERT INTO QUESTOES_AUDIOS (QUESTAO_ID, AUDIO_ID)
VALUES (1,1);

INSERT INTO QUESTOES_AUDIOS (QUESTAO_ID, AUDIO_ID)
VALUES (1,2);

INSERT INTO QUESTOES_AUDIOS (QUESTAO_ID, AUDIO_ID)
VALUES (2,2);

INSERT INTO QUESTOES_AUDIOS (QUESTAO_ID, AUDIO_ID)
VALUES (3,3);

INSERT INTO QUESTOES_AUDIOS (QUESTAO_ID, AUDIO_ID)
VALUES (4,4);

INSERT INTO QUESTOES_AUDIOS (QUESTAO_ID, AUDIO_ID)
VALUES (1,3);

INSERT INTO USUARIO (ID, DATA_CRIACAO, NOME, EMAIL, SENHA)
VALUES (1, '2020-07-18', 'augusto@gmail.com', 'augusto@gmail.com', '123');

INSERT INTO USUARIO (ID, DATA_CRIACAO, NOME, EMAIL, SENHA)
VALUES (2, '2020-07-18', 'marcelo@gmail.com', 'marcelo@gmail.com', '123');

INSERT INTO USUARIO_QUESTAO (DATA_ATUALIZACAO, PONTUACAO, QUESTAO_ID, USUARIO_ID)
VALUES ('2020-08-29', 10, 1, 1);
INSERT INTO USUARIO_QUESTAO (DATA_ATUALIZACAO, PONTUACAO, QUESTAO_ID, USUARIO_ID)
VALUES ('2020-08-29', 10, 2, 1);
INSERT INTO USUARIO_QUESTAO (DATA_ATUALIZACAO, PONTUACAO, QUESTAO_ID, USUARIO_ID)
VALUES ('2020-08-29', 10, 3, 1);

INSERT INTO USUARIO_QUESTAO (DATA_ATUALIZACAO, PONTUACAO, QUESTAO_ID, USUARIO_ID)
VALUES ('2020-08-29', 10, 1, 2);
INSERT INTO USUARIO_QUESTAO (DATA_ATUALIZACAO, PONTUACAO, QUESTAO_ID, USUARIO_ID)
VALUES ('2020-08-29', 10, 2, 2);


SELECT * FROM USUARIO WHERE EMAIL = 'augusto@gmail.com' AND SENHA = '123';